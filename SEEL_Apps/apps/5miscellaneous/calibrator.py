			...
			for a in self.INPUTS:
				if self.I.analogInputSources[a].gainEnabled:
					for b in range(8):
						...
						self.parent.curves[a][b].setData(np.array(self.ADC24['AIN6'])[self.ADC_ACTUALS[a][0]],np.array(self.ADC_VALUES[a][0]) )
		#-------------------------------------------------------
...
class AppWindow(QtGui.QMainWindow, calibrator.Ui_MainWindow,utilitiesClass):
	...
	def selectDir(self):
		...
		if dirname:
			self.savedir = os.path.join(dirname,self.hexid)
			print(self.savedir)
			self.dirnameLabel.setText(self.savedir)
